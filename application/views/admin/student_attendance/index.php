									...
									<div>
	          							...
	          							<form class="form-inline" method="GET">
										    <div class="form-group">
										      <a  data-toggle="modal" data-target="#modal-create" href="" class=" btn btn-primary form-control" title="Add Student Attendance" style="border-radius: 0px"><i class="fa fa-plus" aria-hidden="true"></i> Add Student Attendance</a>
										    </div>


										    <div class="form-group">
			      								<select class="form-control" name="class_id" onchange="this.form.submit()">
			      									<option disabled="disabled" selected="selected">Select a Class</option>
													 ...
													 <tr>
														...
														<?php
															...
															for($date_day=1;$date_day<=$date_limit;$date_day++)
															{
																...
																$attendance_query="SELECT* FROM attendance WHERE student_id='".$row['student_id']."' AND date='".$date."'  AND status=1";
																$query = $this->db->query($attendance_query);

																if ($query->num_rows()>0) {$total+=1;}
															?>
															<td id="attendance-status<?php echo $row['student_id'].'-'.$date_day ?>"><input disabled type="checkbox" <?php echo $query->num_rows()>0 ?'checked':'' ?> ></td>
						              ...
						              <div class="modal-dialog">
						                <div class="modal-content">
						                  ...
						                  <div class="modal-body">
						                        ...
						                        <form method="POST" action="<?php echo site_url(); ?>admin/student_attendance/store/" accept-charset="UTF-8" class="form-horizontal" enctype="multipart/form-data">

	                                                    <div class="form-group ">
	                                                        <label for="name" class="col-md-4 control-label">Class</label>
	                                                        <div class="col-md-6">
	                                                            <select class="form-control" name="class_id"  onchange="get_attendence_form(this.value)">
