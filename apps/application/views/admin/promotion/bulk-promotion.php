								...
								<div class="col-md-12">
									<div>
	          							...
	          							<form class="form-inline" method="GET">
										    ...
										    <div class="form-group">
			      								<select class="form-control col-sm-2" disabled="" name="dest_class_id" id="dest_class_id" required="required" onchange="get_exam_list_by_session_class(this.value, $('#class_id').val())">
			      									<option value="">Promoted to Class</option>
									...
									<form method="post" action="<?php echo site_url() ?>admin/promotion/bulk_promotion">

										<input type="number" name="session_id" hidden value="<?php echo $this->input->get('session_id'); ?>" >
										<input type="number" name="dest_class_id" hidden value="<?php echo $this->input->get('dest_class_id'); ?>" >
									...
									<table class="table table-bordered table-striped table-hover">
										...
										?>
												 <tr <?php if($mark_total	/	$full_mark_total	*	100 < 34) { echo 'style="background-color: #f2d57c;"'; } ?>>
													...
													<td>
														<?php echo $this->class_model->get_class_name_by_id( $this->input->get('dest_class_id'));?>
													</td>
													<td>
														<?php if($mark_total	/	$full_mark_total	*	100 >=34): ?>
														<i class="fa fa-check-circle" style="font-size:24px; color:green"></i>
														<?php elseif($mark_total	/	$full_mark_total	*	100 < 34): ?>
														<i class="fa fa-times-circle" style="font-size:24px; color:darkred"></i>
														<?php endif; ?>

													<input type="number" name="student_id[]" hidden value="<?php echo $student['student_id']; ?>" >
													<input type="number" name="promote[]" hidden value="<?php if($mark_total	/	$full_mark_total	*	100 >=34){echo 1;} else{echo 0;} ?>" >
													</td>
	...
	function get_session_list(val)
	{
		...
		$.ajax({
			...
			success: function(data){
				...
				$('#dest_class_id').removeAttr('disabled');
			}
		});
	}
</script>
