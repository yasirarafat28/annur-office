...
if(($row['userStatus']==1) && ($row['doctorStatus']==2))
{
	echo "Cancelled by Doctor";
							...
							<?php if((($row['userStatus']==1) && ($row['doctorStatus']==0)) || (($row['userStatus']==1) && ($row['doctorStatus']==1)))
{ ?>


	<a href="<?php echo site_url(); ?>patient/appointment_history/?id=<?php echo $row['id']?>&cancel=update" onClick="return confirm('Are you sure you want to cancel this appointment ?')"class="btn btn-transparent btn-xs tooltips" title="Cancel Appointment" tooltip-placement="top" tooltip="Remove">Cancel</a>
	<?php } else {
