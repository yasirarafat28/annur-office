...
class Login extends CI_Controller {
    ...
    public function index() {
        ...
        if ($this->input->post()) {
            ...
            if ($this->input->post('login_level') == 'admin') {
                $login_data = $this->db->get_where('admin', array(
                    'username' => $username,
                    'password' => $password
                ));
            }
            elseif ($this->input->post('login_level') == 'student') {
                $login_data = $this->db->get_where('student', array(
                    'username' => $username,
                    'password' => $password
                ));
            }
            elseif ($this->input->post('login_level') == 'teacher') {
                $login_data = $this->db->get_where('teacher', array(
                    'username' => $username,
                    'password' => $password
                ));
            }
